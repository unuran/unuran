...
sub get_test_distributions
{
	...
	$list_distr->{"$distr\_$n_distr"} = <<EOX;
\t{
\t\t$fpm;
\t\tdistr = unur_distr_$distr(fpm,$n_params);
\t\tif (distr == NULL) {
\t\t\tfprintf( stderr,\"error: cannot make distribution object for $distr($params)\\n\\n\");
\t\t\texit (EXIT_FAILURE);
\t\t}
\t}
...
sub get_test_methods
{
	...
	$t =~ /PAR:\s*(par\s*=\s*unur\_([a-z]+)\_new.+)$/ 
	    or next;

	# Store data
	$list_gen->{"$2\_$n_gen"} = $1;
