 ...
 *   FILE: expect.c                                                        *
 *                                                                           *
 *   Routines for calculations of expectation values of integrals            *
 *                                                                           *
 *****************************************************************************
...
#include "expect_source.h"
#include "../src/methods/x_gen.h"
#endif
/*---------------------------------------------------------------------------*/

#define UNUR_EXPECT_DEFAULT_N_SAMPLES (1e4)
#define UNUR_EXPECT_DEFAULT_N_REPETITIONS (100)

/*---------------------------------------------------------------------------*/

int
_unur_expect_evaluate_single (int dim, struct unur_funct_vgeneric *f, UNUR_GEN *gen, 
     ...
     /*----------------------------------------------------------------------*/
{
  ...
  /* checking input */
  if (n_samples <= 0) {
    n_samples = UNUR_EXPECT_DEFAULT_N_SAMPLES; 
  ...
  *integral = sum / n_samples;
  ...
  return UNUR_SUCCESS;  
} /* end of _unur_expect_evaluate_single() */

/*---------------------------------------------------------------------------*/

int
_unur_expect_evaluate (int dim, struct unur_funct_vgeneric *f, UNUR_GEN *gen, 
     ...
     /*----------------------------------------------------------------------*/
{
  ...
  if (n_repetitions <= 1) {
    n_repetitions = UNUR_EXPECT_DEFAULT_N_REPETITIONS; 
  ...
  *mean = 0;
  for (i=0; i<n_repetitions; i++) {
    _unur_expect_evaluate_single (dim, f, gen, n_samples, &integral[i]);
    *mean += integral[i];
  }
  *mean /= n_repetitions;
