 ...
 *         (Generalized Simple universal generator, ratio-of-uniforms method)*
 *                                                                           *
 *   USAGE:                                                                  *
...
/* 
   =XXMETHOD  GSROU   Generalized Simple Ratio-Of-Uniforms method
   ...
   =DESCRIPTION
      GSROU is based on the generalized ratio-of-uniforms method but
      uses universal inequalities for constructing a (universal)
      bounding rectangle. It works for all T-concave distributions.
      
      It requires the PDF, the (exact) location of the mode and the
      area below the given PDF. Moreover an (optional) parameter
      @code{r} can be given, to adjust generator to the given
      ...
      @code{r}. It is higher for higher values of @code{r}.

      Optionally the CDF at the mode can be given to increase the
      performance of the algorithm by means of the
      unur_gsrou_set_cdfatmode() call. Then the rejection constant is
      reduced by a one half.
      ...
      rejection constant). But then unur_gsrou_set_cdfatmode() must
      not be used.
      ...
      lower bound of the PDF at the mode. Again using
      unur_gsrou_set_cdfatmode() is not allowed.
      
      It is possible to change the parameters and the domain of the
      chosen distribution without building a new generator object
      using the unur_gsrou_chg_pdfparams() and unur_gsrou_chg_domain()
      call, respectively. But then unur_gsrou_chg_pdfarea(),
      unur_gsrou_chg_mode() and unur_gsrou_chg_cdfatmode() have to be
      used to reset the corresponding figures whenever they have
      changed. If any of mode, CDF at the mode, or
      the area below the mode has been changed, then
      unur_gsrou_reinit() must be executed. 
      ...
      switched on by calling unur_gsrou_set_verify() and
      unur_gsrou_chg_verify(), respectively.
...
UNUR_PAR *unur_gsrou_new( UNUR_DISTR *distribution );
/* 
...
int unur_gsrou_reinit( UNUR_GEN *generator );
/* 
...
int unur_gsrou_set_r( UNUR_PAR *parameters, double r );
/* 
...
int unur_gsrou_set_cdfatmode( UNUR_PAR *parameters, double Fmode );
/* 
   Set CDF at mode. 
   ...
   unur_gsrou_chg_cdfatmode() has to be used to update this value.

   Default: not set.
*/

int unur_gsrou_set_usemirror( UNUR_PAR *parameters, int usemirror );
/* 
...
int unur_gsrou_set_verify( UNUR_PAR *parameters, int verify );
/* */

int unur_gsrou_chg_verify( UNUR_GEN *generator, int verify );
/* 
   Turn verifying of algorithm while sampling on/off.
   If the condition (@i{x}) <= hat(@i{x}) is
...
int unur_gsrou_chg_pdfparams( UNUR_GEN *generator, double *params, int n_params );
/* 
...
int unur_gsrou_chg_domain( UNUR_GEN *generator, double left, double right );
/* 
   ...
   changed, then a correspondig unur_gsrou_chg_mode() is required.
   (There is no checking whether the domain is set or not as in the
   unur_init() call.)
*/

int unur_gsrou_chg_mode( UNUR_GEN *generator, double mode );
/* 
   Change mode of distribution.
   unur_gsrou_reinit() must be executed before sampling from the 
   generator again.
*/

int unur_gsrou_upd_mode( UNUR_GEN *generator );
/* 
...
int unur_gsrou_chg_cdfatmode( UNUR_GEN *generator, double Fmode );
/* 
...
int unur_gsrou_chg_pdfarea( UNUR_GEN *generator, double area );
/* 
...
int unur_gsrou_upd_pdfarea( UNUR_GEN *generator );
/*
